...
import { DefenceWireViewModel } from '../viewmodels/DefenceWireViewModel';
...
export function renderDefenceWirePage(): void {
  const viewModel = new DefenceWireViewModel();
    ...
    <main class="main-content">
      <section class="container hero">
        ...
        <div style="background: rgba(34, 197, 94, 0.15); border: 1px solid rgba(34, 197, 94, 0.4); border-radius: 8px; padding: 1rem; max-width: 600px; margin: 0 auto 2rem; color: #4ade80;">
          <strong>${strings.defencewire.privacyBannerLabel}</strong> ${app.privacyGuarantee}
        </div>
        ${
          app.webUrl
            ? `
          <div style="margin-top: 1rem;">
            <a href="${app.webUrl}" target="_blank" rel="noopener noreferrer" class="btn btn-primary" style="display: inline-flex; align-items: center; gap: 0.5rem;">
              <span>${strings.defencewire.launchButton}</span>
            </a>
          </div>
        `
            : ''
      ...
      <section class="container section">
        <h2 class="section-title text-center mb-xl">${strings.defencewire.keyFeaturesTitle}</h2>
        <div class="grid-3">
          ${app.features
            .map(
              (feature) => `
...
if (typeof window !== 'undefined') {
  document.addEventListener('DOMContentLoaded', () => renderDefenceWirePage());
}
