...
import { YogaOfEatingViewModel } from '../viewmodels/YogaOfEatingViewModel';
...
export function renderYogaOfEatingPage(): void {
  const viewModel = new YogaOfEatingViewModel();
    ...
    <main class="main-content">
      <section class="container hero">
        ...
        <div style="background: rgba(34, 197, 94, 0.15); border: 1px solid rgba(34, 197, 94, 0.4); border-radius: 8px; padding: 1rem; max-width: 600px; margin: 0 auto 2rem; color: #4ade80;">
          <strong>${strings.yogaOfEating.privacyBannerLabel}</strong> ${app.privacyGuarantee}
        </div>
      </section>

      <section class="container section">
        <h2 class="section-title text-center mb-xl">${strings.yogaOfEating.keyFeaturesTitle}</h2>
        <div class="grid-3">
          ${app.features
            .map(
              (feature) => `
...
if (typeof window !== 'undefined') {
  document.addEventListener('DOMContentLoaded', () => renderYogaOfEatingPage());
}
